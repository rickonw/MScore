...
static inline int wmidi_open_write (const char *path, std::fstream &fd) {
	...
	fd.open(path, (O_RDWR | O_CREAT | O_TRUNC | O_BINARY ), 0664);
	if (!fd.is_open()) {
		return false;
	}
...
static inline void wmidi_close (std::fstream &fd) {
    //_close(fd);
	fd.close();
}
static inline void wmidi_seekset (std::fstream &fd, long ofs) {
	//streampos
	fd.seekp(ofs, ios::beg);
    //return _lseek(fd, ofs, SEEK_SET);
}
static inline int wmidi_write (std::fstream &fd, const void *buf, size_t size) {
 ...
 */
    ...
    if (!wmidi_open_write(midi_file, audio_fd)) {
...
static int open_wav_output(void) {
    ...
    if (!wmidi_open_write(wav_file, audio_fd)) {
        fprintf(stderr, "Error: unable to open file for writing (%s)\r\n", strerror(wmidi_geterrno()));
        return (-1);
    } else {
...
void wildmidi_pause(int inpause) 
{
    if (inpause) {
        fprintf(stderr, "       \r");
        resume_output();
...
int wildmidi_play(void *midi_ptr, struct _WM_inPlayHook *wmhook) 
{
    struct _WM_Info *wm_info;
    int i, res;
    int option_index = 0;
	...
	struct _WM_inPlayPara PlayPara;
    ...
    memset(lyrics,' ',MAX_LYRIC_CHAR);
    memset(display_lyrics,' ',MAX_DISPLAY_LYRICS);
    memset(&PlayPara, 0, sizeof(PlayPara));
    wm_info = WildMidi_GetInfo(midi_ptr);
    ...
    while (1) {
        ...
        wmhook->pfn_get_para(midi_ptr, &PlayPara);

        if(PlayPara.pause_set) {
            wildmidi_pause(PlayPara.pause_state);
            PlayPara.pause_set = false;
        }
    
        if (PlayPara.pause_state) {
            wm_info = WildMidi_GetInfo(midi_ptr);
            perc_play = (wm_info->current_sample * 100)
                        / wm_info->approx_total_samples;
            pro_mins = wm_info->current_sample / (rate * 60);
        ...
        wmhook->pfn_update(wmhook->handle, wm_info->current_sample, 
                           wm_info->approx_total_samples, wm_info->total_midi_time);
...
int wildmidi(const char *file, char *cfg, struct _WM_inPlayHook *wmhook)
{
    struct _WM_Info *wm_info;
    int i, res;
    int option_index = 0;
    ...
    strncpy(config_file, cfg, sizeof(config_file));
    config_file[sizeof(config_file) - 1] = 0;
