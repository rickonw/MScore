...
struct _WM_inPlayPara {
    uint32_t pause_state;
    bool pause_set;
    uint16_t mixer_options;
};

struct _WM_inPlayHook
{
    void *handle;

    /* hooks */
    void (*pfn_get_para)(void *midi, struct _WM_inPlayPara *val);
    void (*pfn_update)(void *handle, uint32_t current, 
                       uint32_t total, uint32_t total_midi_time);
};
